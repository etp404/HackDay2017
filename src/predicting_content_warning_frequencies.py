...
csv_reader_test = csv.reader(open('transformed_training_data copy 2.csv', 'r'))
trainLabels=[]
trainMp3Content = ndarray((0,3387))
i=0
for row in csv_reader_test:
    trainLabels.append(float(row[1]))
    trainAsarray = numpy.asarray(row[2:], float)
...
csv_reader_train = csv.reader(open('transformed_test_data copy 2.csv', 'r'))
testLabels=[]
testMp3Content = ndarray((0,3387))
i=0
for row in csv_reader_train:
    testLabels.append(float(row[1]))
    testAsarray = numpy.asarray(row[2:], float)
